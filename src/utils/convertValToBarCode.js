function f_barra() {
  ...
  if (antes != depois && antes != "")
    alert("O código de barras digitado não confere:\n" + antes + "\n" + depois);
  f_venc();
  return false;
}
function f_linha() {
...
function f_venc() {
  ...
  else form.venc.value = fator_vencimento(form.barra.value.substr(5, 4));
  form.valor.value =
    form.barra.value.substr(9, 8) * 1 + "," + form.barra.value.substr(17, 2);
  return false;
}
export function calcula_barra() {
  const linhaDigitavel = "03399569638100000262634763601019493660000020120";
  //var linha = form.linha.value; // Linha Digitável
  barra = linhaDigitavel.replace(/[^0-9]/g, "");
  //
...
function calcula_linha(barra) {
  ...
  //
  var campo1 =
    linha.substr(0, 4) + linha.substr(19, 1) + "." + linha.substr(20, 4);
